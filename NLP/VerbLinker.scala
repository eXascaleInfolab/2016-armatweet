...
import tweetsUtils.Tweet.VerbParticle
...
class VerbLinker {
  ...
  //  val wnDir = classOf[VerbLinker].getResource("wordnetdict31")
  ...
  def linkVerb(tweetWithPOS: Array[(String, String)], idx: Int) = {
  ...
  /**
    * Links all verbs in tweet.
    *
    * @param tweet
    * @return a sequence of optional quadruples (startoffset, endoffset, verb lemma, wn synset)
    */
  def linkVerbs_offsets(tweet: Tweet): IndexedSeq[Option[(Int, Int, String, Synset)]] = {
    val verbsIdx = tweet.tokens.zipWithIndex.filter { case (tok, idx) => tok.tokType == Tweet.Verb }

    verbsIdx.map { case (vTok, idx) =>
      val baseVerb = NtRE.replaceAllIn(vTok.token, "")

      val currentBest = bestSynsetFor(baseVerb)
      val bestSynset = currentBest match {
        //composite verb?
        case Some((verbMorphy, oneWordSset))
          if (idx + 1 < tweet.tokens.length &&
            tweet.tokens(idx + 1).tokType == VerbParticle) =>

          val compositeVerb = verbMorphy + "_" + tweet.tokens(idx + 1).token
          val compositeSynset = bestSynsetFor(compositeVerb)

          if (compositeSynset.nonEmpty) {
            val Some((verbLemma, synset)) = compositeSynset
            val (offsetS, offsetE) = (vTok.start, tweet.tokens(idx + 1).end)
            Some((offsetS, offsetE, verbLemma, synset))
          } else {
            Some((vTok.start, vTok.end, verbMorphy, oneWordSset))
          }

        // not composite this time
        case Some((verblemma, synset)) =>
          Some((vTok.start, vTok.end, verblemma, synset))
...
//      else println("failed")
  ...
  def linkRelation(triple: RelationTriple): Option[(String, Synset)] = {
    ...
    assert(verbs.length < 2) //assume there is only one verb
